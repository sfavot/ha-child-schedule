...
class ChildScheduleOptionsFlow(OptionsFlow):
    ...
    The edited config is kept in memory during the flow and saved as the
    entry options at the end, which reloads the entry.
    """
    ...
    async def async_step_school(
        ...
        return self.async_show_form(
            step_id="school", data_schema=schema, errors=errors
        )
    ...
    async def async_step_save(
        ...
        """Validate and save the edited schedule."""
        if user_input is not None:
            # Submitting the error form returns to the main menu.
            return await self.async_step_init()
        try:
